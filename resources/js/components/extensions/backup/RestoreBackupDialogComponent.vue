                ...
                <div v-if="data">
                    ...
                    >
                        ...
                        <template v-slot:[`item.used_in_expenses`]="{ item }">
                            <div class="checkbox-centered">
                                <v-checkbox v-model="item.used_in_expenses" disabled></v-checkbox>
                            </div>
                        </template>
                    ...
                    <div class="text-center text-capitalize pb-lg-0 text-h6" :class="$vuetify.theme.dark ? 'white--text' : 'black--text'">Expenses</div>
                    <v-data-table
                        :headers="headers.transactions"
                        :items="data.expenses"
                        :mobile-breakpoint="0"
        ...
        validateReports(reports) {
            ...
            let validationArray = reports.reports.map(item => [
                ...
                Array.isArray(item.queries) && item.queries.map(item1 => [
                    ["income", "expenses"].includes(item1.query_data),
        ...
        file() {
            ...
            this.file.text()
                ...
                .then(data => {
                    ...
                    if (!this.validateTransactions(data.income)) {
                        throw new Error("Invalid income");
                    }

                    if (!this.validateTransactions(data.expenses)) {
