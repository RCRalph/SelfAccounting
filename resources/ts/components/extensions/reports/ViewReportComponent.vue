                                ...
                                <template v-slot:top>
                                    <v-row class="align-center mb-0">
                                        ...
                                        <v-col cols="12" sm="7" lg="8"
                                               :order="$vuetify.breakpoint.xsOnly ? 'first' : 'last'" class="d-flex"
                                ...
                                <template v-slot:item="{item, index}">
                                    <tr class="text-center">
                                        ...
                                        <td
                                            ...
                                            @mouseleave="tableData.resetHoveredRows()"
                                        >{{
                                                item.price.value | addSpaces
                                            }}&nbsp;{{ currencies.findCurrency(item.currency_id.value).ISO }}
                                        </td>
                                        ...
                                        >
                                            <div class="d-flex justify-start align-center">
                                                <div class="mr-2">
                                                    <v-icon style="min-width: 24px" v-if="item.category.value.icon">
                                                        {{ item.category.value.icon }}
                                                    </v-icon>
                                                </div>
                                        ...
                                        >
                                            <div class="d-flex justify-start align-center">
                                                <div class="mr-2">
                                                    <v-icon style="min-width: 24px" v-if="item.account.value.icon">
                                                        {{ item.account.value.icon }}
                                                    </v-icon>
                                                </div>
                        ...
                        <v-row :no-gutters="$vuetify.breakpoint.xl">
                            ...
                            <v-col xl="12" md="4" cols="12" class="mb-xl-4">
                                <v-card style="height: 100%;">
                                    ...
                                    <v-card-text v-if="!information.sum" class="d-flex align-center justify-center"
                                                 style="height: calc(100% - 64px)">
                                    ...
                                    >
                                        {{
                                            information.sum[Object.keys(information.sum)[0]] | addSpaces
                                        }}&nbsp;{{ currencies.findCurrency(Object.keys(information.sum)[0]).ISO }}
                                    </v-card-text>
                                ...
                                <v-card style="height: 100%;">
                                    ...
                                    <v-card-text class="d-flex align-center justify-center"
                                        ...
                                        <div class="d-flex justify-space-around flex-wrap">
                                            <v-btn outlined class="mx-2 my-1" @click="exportToCSV" width="172">Export to
                                                .csv
                                            </v-btn>

                                            <v-btn outlined class="mx-2 my-1" @click="exportToXLSX" width="172"
                                                   color="success">Export to .xlsx
                                            </v-btn>
                                        </div>
                                    </v-card-text>
                                </v-card>
    ...
    computed: {
        items() {
            ...
            if (this.options.sortBy.length) {
                items.sort(this.sortUsingOptions());
            }
                ...
                if (data[0].includes(column)) {
                    ...
                    for (let i = range.s.r + 1; i <= range.e.r; i++) {
                        sheet[XLSX.utils.encode_cell({
                            r: i,
                            c: columnIndex
                        })].z = `0.00 ${this.currencies.findCurrency(this.tableData.tabulatedData[i - 1].currency_id).ISO}`;
                    }
                }
            });
