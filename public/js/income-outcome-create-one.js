...
jQuery(function () {
  ...
  $(".value-counting").on("input", function () {
    $("#value").val(Math.round($("#price").val() * $("#amount").val() * 100) / 100);
  });
  $("#currency_id").on("change", function () {
    var currentCurrency = $("#currency_id").val();
    $("#category_id, #mean_id").find("option").remove();

    if (categoriesAndMeans.categories[currentCurrency]) {
      for (var _i = 0, _Object$entries = Object.entries(categoriesAndMeans.categories[currentCurrency]); _i < _Object$entries.length; _i++) {
    ...
    if (categoriesAndMeans.means[currentCurrency]) {
      for (var _i2 = 0, _Object$entries2 = Object.entries(categoriesAndMeans.means[currentCurrency]); _i2 < _Object$entries2.length; _i2++) {
        var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2),
            _id = _Object$entries2$_i[0],
            _name = _Object$entries2$_i[1];

        $("#mean_id").append('<option value="' + _id + '">' + _name + '</option>');
      }
    }

    $("#category_id, #mean_id").append('<option value="null">N / A</option>');
  });
});
